...
> **Dynamic deny rules.** Per the post-`discovery-first-default` pivot (§A21), the v1.0+ ergonomics for cross-product opt-in denies are conversational dynamic deny rules: `iam-jit deny add --target 'arn:aws:s3:::prod-*' --duration 3h` fans out across every applicable bouncer + embeds the same constraint into iam-jit-issued roles. **ibounce now consumes the cross-product YAML directly** (`~/.iam-jit/dynamic-denies.yaml`, override via `--dynamic-denies-path` / `IAM_JIT_DYNAMIC_DENIES_PATH`) — schema-validated loader, fsevents/inotify hot reload, ARN matcher, `/admin/dynamic-denies/reload` mgmt-port endpoint, OCSF audit events surfacing `unmapped.iam_jit.ext.dynamic_deny_rule_id`. The unified `iam-jit deny add | list | remove | show` CLI + MCP fan-out land in #324e; recommender `Deny`-injection lands in #324f. See [docs/DYNAMIC-DENY-RULES.md](docs/DYNAMIC-DENY-RULES.md) for the canonical design + on-disk YAML shape.

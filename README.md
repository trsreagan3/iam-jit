...
- **[docs/ORG-PROFILE-DISTRIBUTION.md](docs/ORG-PROFILE-DISTRIBUTION.md)** — distribute safety policy across your org via `ibounce profile install --from URL --sha256 <hex>` (v1.0 shipped); bundle index format, engineer onboarding, override semantics, CI/CD integration. Pairs with five starter profiles under `docs/examples/profiles/`. The unified `bounce init --org-url` + `bounce profile sync` surface is PLANNED for v1.1; the doc calls out shipped-vs-planned per command.

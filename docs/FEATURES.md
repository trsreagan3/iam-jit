...
This is the full feature catalog for iam-jit + the Bounce suite, extracted
from the historical long-form README per the docs cleanup sweep
(MED-7 + LOW-1, 2026-05-25). The top-level [`../README.md`](../README.md)
remains the short product overview; this file is the long-form reference
for what every product ships.

If you want install steps, see [`GETTING-STARTED.md`](GETTING-STARTED.md).
If you want the v1.0 platform-features cluster (declarative config,
autopilot, anomaly detection, etc.) it lives at the end of this file.
...
> **No multi-tenant hosted SaaS.** iam-jit-the-company does not operate any shared infrastructure tier. The iam-risk-score scorer is an **offline** CLI + library (no hosted API; runs in your process). The other three products run on your laptop or in your own AWS account. Running a tool that holds trust roles into many customer AWS accounts would create a SolarWinds-style blast radius we refuse to host. Dedicated single-tenant managed Enterprise contracts are available for large customers at high-fee — but each deployment is fully isolated, not shared. (The previously-hosted `iam-risk-score.com` API was dropped 2026-05-24 to restore this stance to 100%.)

> **What "ships in v1.0" means.** All four products are complete in v1.0, including the `ibounce` HTTP proxy. The transparent HTTP-proxy interception (`ibounce run` redirecting `AWS_ENDPOINT_URL` traffic) is shipped with both cooperative + transparent modes; environment profiles as a hard-floor deny layer (cross-product defaults reduced to `full-user` + `safe-default` in v1.0; `safe-default` uses a `policy_sentry`-backed readonly-admin-minus baseline, not enumerated destructive verbs); `bouncer pause --for 30m` as a timed escape hatch; `bouncer prompts` as an async deny-notification queue; and HTTPS install of org-distributed profiles. See [IBOUNCE.md](IBOUNCE.md) for details.

> **Bounce-suite rename (v1.0, 2026-05-17).** What used to be `iam-jit-bouncer` is now `ibounce` — the canonical name across the Bounce family (ibounce + kbounce + future). The `iam-jit-bouncer` console script keeps working for v1.0 (prints a deprecation warning + forwards to the same entrypoint) and is removed in v1.1. Every `bouncer_*` MCP tool gets an `ibounce_*` alias; both names dispatch to the same handler in v1.0. See [UPGRADING.md](UPGRADING.md) for the one-line migration.

> **Dynamic deny rules.** Per the post-`discovery-first-default` pivot (§A21), the v1.0+ ergonomics for cross-product opt-in denies are conversational dynamic deny rules: `iam-jit deny add --target 'arn:aws:s3:::prod-*' --duration 3h` fans out across every applicable bouncer + (per #324f) embeds the same constraint into iam-jit-issued roles. **All 4 bouncers + the unified `iam-jit deny` CLI + the `bounce_deny_*` MCP tools are SHIPPED (#324a/b/c/d/e, 2026-05-22).** ibounce + kbounce + dbounce + gbounce each consume the cross-product YAML directly (`~/.iam-jit/dynamic-denies.yaml`, override via `IAM_JIT_DYNAMIC_DENIES_PATH`) — schema-validated loaders, fsevents/inotify hot reload, per-protocol matchers, `/admin/dynamic-denies/reload` mgmt-port endpoints, OCSF audit events. The unified CLI resolves target shape to the right bouncer(s) (ARN -> ibounce, `namespace:`/`cluster:` -> kbounce, `rds:`/RDS-endpoint -> dbounce+gbounce, URL/hostname -> gbounce), atomically writes the YAML at 0600, and POSTs each affected bouncer's reload endpoint. Unreachable bouncers are surfaced honestly + the CLI still exits 0 — the YAML file IS the source of truth, the bouncer's watcher picks the rule up on next start. Recommender `Deny`-injection (defense-in-depth in iam-jit-issued roles) lands in #324f. See [DYNAMIC-DENY-RULES.md](DYNAMIC-DENY-RULES.md).
...
> Score AWS IAM policies before you grant access. Deterministic offline engine, 1–10 risk score, sub-100ms response — no hosted API, no phone-home, no credentials sent anywhere.
...
> **No hosted scoring API.** The previously-hosted `api.iam-risk-score.com` endpoint was dropped on 2026-05-24 to restore `[[no-hosted-saas]]` to 100%. The deterministic scorer is the moat; the offline CLI + library are the supported access surface.
...
The 1–10 scale is **adversarially calibrated**: every scoring rule is pinned by tests sourced from real attack patterns + the full AWS-managed-policy corpus. See [CONVERGENCE-REPORT-2026-05.md](CONVERGENCE-REPORT-2026-05.md) and [ADVERSARIAL-LOOP-PROCESS.md](ADVERSARIAL-LOOP-PROCESS.md).
...
`--apply`. See [PROFILE-UPGRADE.md](PROFILE-UPGRADE.md)
...
- **Environment profiles**: named, switchable hard-floor deny layers; cross-product built-in defaults are `full-user` (passthrough, default-active) + `safe-default` (AWS readonly admin baseline minus sensitive reads — uses `policy_sentry`'s Read+List classifications as the allow-baseline + a curated subtract list of sensitive Read actions; supersedes the v1.0-alpha `readonly` deny-verb shape per the `safe_default_is_readonly_admin_minus` memo). Opt in with `--profile safe-default` OR `export IAM_JIT_BOUNCER_PROFILE=safe-default` in your shell rc. **What it covers:** state-changing AWS operations (writes, privilege grants like `iam:PassRole`/`iam:Attach*Policy`, credential minting like `sts:AssumeRole`, code execution like `lambda:InvokeFunction`/`ssm:SendCommand`) are denied by construction (not in Read+List baseline). **What it does NOT cover:** this is NOT a confidentiality boundary — the baseline allows reads of S3 objects, RDS data, CloudWatch logs, IAM metadata. Pair with bucket policies / KMS grants / column-masking for confidentiality. Community profiles (`staging-work`, `dev-only`, `incident-response`, etc.) installable via `profile install --from URL` from `tools/community-profiles/` (future: `trsreagan3/bounce-profiles`)
...
See [IBOUNCE.md](IBOUNCE.md) for full CLI + MCP reference, task-scope composition rules, and the recommender workflow.

### Known limitations

Read before you install — knowing the boundaries up-front saves debugging time. The canonical list lives at [`KNOWN-CAVEATS.md`](KNOWN-CAVEATS.md); the top three for ibounce are:
...
**iam-jit v1.0 ships fully free + open source.** Every feature described in this document ships in the Apache-2.0 release: scoring engine, audit log, MCP server, OIDC SSO, MFA propagation, Slack approval bot, all four bouncers, deny/anomaly/autopilot tooling — no per-feature gating, no paid tier at launch.

Self-host in your own AWS account (no multi-tenant SaaS — see below). Zero phone-home. Customer-chosen LLM backend (Bedrock / Anthropic / OpenAI / Ollama / none) when standalone-mode LLM is desired; local-dev with an agent in the loop needs **zero LLM credentials on the iam-jit/bouncer side** — the agent's own LLM does the intelligent work via MCP. See [`LLM-BACKENDS.md`](LLM-BACKENDS.md).
...
See [GETTING-STARTED.md](GETTING-STARTED.md) for the full walkthrough — ~5 minutes to a working MVP deployment.
...
See [security/](security/) for the BB+WB audit history (19+ rounds shipped) and [compliance/](compliance/) for the framework mapping.
...
`ibounce` automatically rotates `audit.jsonl` at 100 MB or 7 days (whichever first), gzips the archive, and tracks freshness + disk usage. See [LOG-RETENTION.md](LOG-RETENTION.md) for the cross-product runbook covering `ibounce logs purge / archive / verify`, `ibounce doctor logs`, the `/healthz` disk-degraded signal, and the operator runbook for "audit log degraded" alerts. The same surface ships on kbounce / dbounce / gbounce — with one caveat: **gbounce LogWriter-level rotation is deferred to v1.1 per [KNOWN-CAVEATS.md §A10](KNOWN-CAVEATS.md). gbounce ships the rotation primitives + CLI surface but the WRITER-side rotation hook is pending; the other 3 bouncers rotate end-to-end today.**
...
See [compliance/](compliance/) for the full framework mapping.
...
Once you've shipped bouncers to engineers' laptops + CI runners, the next question is *"how do I keep the safety policy consistent across the fleet?"* The Bounce suite ships a pull-based distribution model: your security team publishes one or more profile YAMLs at an HTTPS URL your org controls; every engineer runs `ibounce profile install --from URL --sha256 <hex>` on day 1 (v1.0 shipped surface) to install each profile with sha256-pinned integrity. Installed-from-URL profiles are read-only at the CLI surface — engineers can add denies on top but cannot peel back the org floor. The unified `bounce init --org-url ...` + `bounce profile sync` (multi-profile bootstrap + ETag-based update polling) are PLANNED for v1.1 — see [ORG-PROFILE-DISTRIBUTION.md](ORG-PROFILE-DISTRIBUTION.md) for the shipped-vs-planned breakdown.
...
Distinct from `[[no-nl-synthesis]]` (IAM-policy NL synthesis remains forbidden) — bounce profiles are operator-reviewable config artifacts, not security-boundary policies. See [PROFILE-GENERATION.md](PROFILE-GENERATION.md) for the full agent + operator guide.
...
See [AGENTS.md](AGENTS.md) for the agent-driven reduction-loop pattern in detail.
...
See [../CHANGELOG.md](../CHANGELOG.md) for release history and [ROADMAP-V1.1.md](ROADMAP-V1.1.md) for post-launch scope.

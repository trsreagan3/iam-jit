...
TOOLS.extend([
    {
        ...
        "description": (
            ...
            "role(s)/profile(s), AWS services + resource ARNs, K8s "
            "namespaces, databases, external HTTP endpoints, and MCP "
            "tools. Output is a valid CycloneDX 1.6 JSON doc "
            "(`bomFormat`/`specVersion`/`serialNumber`/`metadata`/"
            "`components`) ingestible by Dependency-Track or "
